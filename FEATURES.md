...
| Journeyman product page | Visitor opens `/journeyman` | `src/pages/journeyman.astro` | Build and read `dist/journeyman/index.html`; Rust & Iron theme via `<body data-theme="journeyman">`. Sections: what the product is, the askable library (the flagship surface, skinned per customer and isolated per plant), how an engagement runs, who it fits, and the two fee halves (capture quoted per project, library monthly from delivery) |
